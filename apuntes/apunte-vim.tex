...
\begin{tabular}{ l l }
	...
	\texttt{fa} & saltar a la próxima ocurrencia de la letra a en la linea. \\ & Adicionalmente ``,''  (o ``;'') encontrará la próxima ocurrencia o la previa \\
...
Un tip útil es: \texttt{dt"}  el cual borra todo hasta el caracter \texttt{"}.  
...
\subsection{Insertar textos}

Dos funciones muy útiles suelen ser poder insertar un archivo de texto en 
el lugar donde se encuentra el cursor e, insertar la salida de un comando
en el lugar donde se encuentra el cursor. Para esto, en modo normal podemos
ejecutar los siguientes comandos:

\begin{itemize}
	\item El comando anterior realiza lo siguiente: \\ \\
\begin{tabular}{ l l }
            \texttt{:!<comando>} & Ejecuta \texttt{<comando>} en el shell y retorna\\ & sin insertar texto en el documento actual. \\
            \texttt{!!<comando>} & Ejecuta  \texttt{<comando>} en el shell y la salida \\ & del mismo se inserta en el documento actual. \\ 
            \texttt{:r <archivo>} & Inserta el contenido de \texttt{<archivo>}  en el \\
 & documento actual.\\
\end{tabular}
\end{itemize}
